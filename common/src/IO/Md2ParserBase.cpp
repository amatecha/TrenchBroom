...
namespace TrenchBroom {
    namespace IO {
        const Vec3f Md2ParserBase::Normals[162] = {
        ...
        Md2ParserBase::Md2Frame::Md2Frame(const size_t vertexCount) :
        vertices(vertexCount) {}

        Vec3f Md2ParserBase::Md2Frame::vertex(const size_t index) const {
        ...
        const Vec3f& Md2ParserBase::Md2Frame::normal(const size_t index) const {
            const Md2Vertex& vertex = vertices[index];
            return Normals[vertex.normalIndex];
        }

        Md2ParserBase::Md2Mesh::Md2Mesh(const int i_vertexCount) :
        type(i_vertexCount < 0 ? Fan : Strip),
        vertexCount(static_cast<size_t>(i_vertexCount < 0 ? -i_vertexCount : i_vertexCount)),
        vertices(vertexCount) {}

        Md2ParserBase::Md2ParserBase(const String& name, const char* begin, const char* end, const Assets::Palette& palette, const FileSystem& fs) :
        m_name(name),
        m_begin(begin),
        m_end(end),
        m_palette(palette),
        ...
        Md2ParserBase::~Md2ParserBase() {}

        Md2ParserBase::Md2SkinList Md2ParserBase::parseSkins(CharArrayReader reader, const size_t skinCount) {
            Md2SkinList skins(skinCount);
            reader.readRawVector(skins);
            return skins;
        }

        Md2ParserBase::Md2FrameList Md2ParserBase::parseFrames(CharArrayReader reader, const size_t frameCount, const size_t frameVertexCount) {
            Md2FrameList frames(frameCount, Md2Frame(frameVertexCount));

            for (size_t i = 0; i < frameCount; ++i) {
                frames[i].scale = reader.readVec<float, 3, float>();
                frames[i].offset = reader.readVec<float, 3, float>();
                frames[i].name = reader.readString(Md2Layout::FrameNameLength);
                reader.readRawVector(frames[i].vertices);
            }
            
            return frames;
        }

        Md2ParserBase::Md2MeshList Md2ParserBase::parseMeshes(CharArrayReader reader, const size_t commandLength) {
            Md2MeshList meshes;

            size_t i = 0;
            while (i < commandLength) {
                const int vertexCount = reader.readInt<int32_t>(); ++i;
                Md2Mesh mesh(vertexCount);
                
                for (size_t j = 0; j < mesh.vertexCount; ++j) {
                    mesh.vertices[j].texCoords[0] = reader.readFloat<float>(); ++i;
                    mesh.vertices[j].texCoords[1] = reader.readFloat<float>(); ++i;
                    mesh.vertices[j].vertexIndex = reader.readSize<int32_t>(); ++i;
                }
        ...
        Assets::EntityModel* Md2ParserBase::buildModel(const Md2SkinList& skins, const Md2FrameList& frames, const Md2MeshList& meshes) {
        ...
        Assets::TextureList Md2ParserBase::loadTextures(const Md2SkinList& skins) {
        ...
        Assets::Texture* Md2ParserBase::readTexture(const Md2Skin& skin) {
        ...
        Assets::Md2Model::FrameList Md2ParserBase::buildFrames(const Md2FrameList& frames, const Md2MeshList& meshes) {
        ...
        Assets::Md2Model::Frame* Md2ParserBase::buildFrame(const Md2Frame& frame, const Md2MeshList& meshes) {
        ...
        Assets::Md2Model::VertexList Md2ParserBase::getVertices(const Md2Frame& frame, const Md2MeshVertexList& meshVertices) const {
